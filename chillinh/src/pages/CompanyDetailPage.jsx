...
import { MdLocationOn, MdAccessTime, MdAttachMoney, MdCalendarToday } from 'react-icons/md';
...
function CompanyDetailPage() {
  ...
  const handleReplyClick = (reviewId, replyUser, replyId) => {
    ...
    setReplyBox(prev => ({ ...prev, [reviewId]: replyUser ? `@${replyUser} ` : '' }));
  };
      ...
      <div className="companyCard">
        ...
        <div className="companyMainInfo">
          ...
          <div className="companyMetaRow">
            ...
            <div className="companyMetaCol">
              <div><b>Quy mô công ty</b></div>
              <div>{company.employees}</div>
            </div>
            <div className="companyMetaCol">
              <div><b>Lĩnh vực</b></div>
      ...
      <div className="container">
          ...
          <div key={job.id} className="jobCard">
            ...
            <div className="jobInfo">
              ...
              <div className="jobMeta">
                <span><span className="jobMetaIcon"><MdLocationOn /></span>{job.location}</span>
                <span className="jobMetaDot">·</span>
                <span><span className="jobMetaIcon"><MdAccessTime /></span>{job.type}</span>
                <span className="jobMetaDot">·</span>
                <span><span className="jobMetaIcon"><MdAttachMoney /></span>{job.salary}</span>
                <span className="jobMetaDot">·</span>
                <span><span className="jobMetaIcon"><MdCalendarToday /></span>{timeAgo(job.posted)}</span>
              </div>
        ...
        <div className="reviewHeaderBar">
          <div style={{display:'flex',alignItems:'center'}}>
            ...
            <span className="reviewStars">{'★'.repeat(Math.round(company.rating))}{'☆'.repeat(5-Math.round(company.rating))}</span>
          </div>
          <div className="reviewFilterBar">
            <button className="reviewFilterBtn selected">Xem các đánh giá có gắn thẻ công ty</button>
          ...
          <div style={{fontStyle:'italic',color:'#222',fontSize:'1.05rem'}}>Dựa trên {company.ratingCount} đánh giá</div>
        </div>
        <form onSubmit={handleCommentSubmit} className="form" style={{marginBottom: 24}}>
          <h4>Viết đánh giá</h4>
          <textarea
            value={comment}
        ...
        {company.reviews && company.reviews.map(review => (
          <div key={review.id} className="reviewCard">
            <div className="reviewAvatar">{review.avatar ? <img src={review.avatar} alt="avatar" style={{width:'100%',height:'100%',borderRadius:'50%'}}/> : (review.user[0])}</div>
            <div className="reviewMain">
              <div className="reviewUserRow">
                <span className="reviewUserName">{review.user}</span>
                <span className="reviewStarsRow">{'★'.repeat(review.rating)}{'☆'.repeat(5-review.rating)}</span>
                ...
                <div className="reviewReplyList">
                  {review.replies.map(reply => (
                    <div key={reply.id} className="reviewReplyCard">
                      <div className="reviewReplyAvatar">{reply.avatar ? <img src={reply.avatar} alt="avatar" style={{width:'100%',height:'100%',borderRadius:'50%'}}/> : (reply.user[0])}</div>
                      <div className="reviewReplyMain">
                        <div className="reviewReplyUserRow">
                          <span className="reviewReplyUserName">{reply.user}</span>
                          <span className="reviewReplyTime">{timeAgo(reply.date)}</span>
                        </div>
                        <div className="reviewReplyContent">{reply.content}</div>
                        <div style={{marginTop: 6}}>
                          {replyingTo && replyingTo.reviewId === review.id && replyingTo.replyId === reply.id ? (
                            <div style={{display:'flex',gap:8,alignItems:'center'}}>
                              <input
                                ...
                                style={{flex:1,padding:'7px 12px',borderRadius:6,border:'1.2px solid #cfd8dc',fontSize:'1rem'}}
                              />
                              <button
                                onClick={() => handleReplySubmit(review.id)}
                                style={{background:'#1976d2',color:'#fff',border:'none',borderRadius:6,padding:'7px 18px',fontWeight:600,cursor:'pointer'}}
                              >Gửi</button>
                              <button
                                onClick={() => setReplyingTo(null)}
                                style={{background:'none',color:'#888',border:'none',fontWeight:500,cursor:'pointer'}}
                              >Huỷ</button>
                            </div>
                          ) : (
                            <button
                              onClick={() => handleReplyClick(review.id, reply.user, reply.id)}
                              style={{background:'none',color:'#1976d2',border:'none',fontWeight:500,cursor:'pointer',marginTop:2}}
                            >Trả lời</button>
                          )}
                        </div>
                      </div>
              ...
              <div style={{marginTop: 10}}>
                {replyingTo && replyingTo.reviewId === review.id && !replyingTo.replyId ? (
                  <div style={{display:'flex',gap:8,alignItems:'center'}}>
                    <input
                      type="text"
                      value={replyBox[review.id] || ''}
